...
The .NET JIT unlinks the memfd file immediately, causing perf to report frames as [unknown].

The embedded Python script performs a "fuzzy" lookup: if start_addr <= sample_addr < start_addr + size.

It replaces the opaque hex addresses with human-readable C# signatures.